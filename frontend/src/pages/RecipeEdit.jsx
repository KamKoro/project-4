...
const RecipeEdit = () => {
  ...
  const updateMutation = useMutation(
    (recipeData) => recipesAPI.updateRecipe(id, recipeData),
    {
      onSuccess: () => {
  ...
  useEffect(() => {
    if (recipe) {
      reset({
        ...
        ingredients: recipe.ingredients.length > 0 ? recipe.ingredients.map(ing => ({
          ingredient_id: ing.ingredient?.id || null,
          amount: ing.amount || '',
          unit: ing.unit || '',
          notes: ing.notes || ''
        })) : [{ ingredient_id: null, amount: '', unit: '', notes: '' }],
        instructions: recipe.instructions.length > 0 ? recipe.instructions : [{ step: '' }]
      });
      if (recipe.image) {
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="card">
          ...
          <div className="mt-4">
            ...
            {imagePreview && (
              <div className="mb-4">
                <img src={imagePreview} alt="Current" className="h-32 w-32 object-cover rounded-lg" />
              </div>
            )}
